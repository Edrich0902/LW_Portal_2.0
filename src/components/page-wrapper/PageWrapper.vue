    ...
    <div v-if="showToolbar" class="w-full flex flex-row justify-between items-center">
      <div class="flex items-center gap-3">
        <button
          ...
          type="button"
          class="flex items-center justify-center w-10 h-10 rounded-lg cursor-pointer text-surface-600 dark:text-surface-300 hover:bg-surface-100 dark:hover:bg-surface-800 transition-colors"
          @click="layout.toggleSidebar()"
        >
          <span class="pi pi-bars text-lg" />
        </button>
        <div class="text-2xl font-bold tracking-tight">{{ title ?? '' }}</div>
      </div>
